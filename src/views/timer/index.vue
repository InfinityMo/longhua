...
export default {
  ...
  methods: {
    downTemplate () {
      downLoadFile('/timeseries/downloadmold')
    },
    uploadHandel (fileList) {
      ...
      this.$request.post('/timeseries/upload', formData, {
        spinning: false,
    ...
    getTableData () {
      ...
      this.$request.post('/timeseries/filelist', searchForm).then(res => {
    ...
    deleteHandle (id) {
      this.$request.post('/timeseries/filedelete', { id }).then(res => {
        if (res) {
